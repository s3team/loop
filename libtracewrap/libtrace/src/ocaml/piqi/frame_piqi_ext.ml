...
let _operand_value_list_piqtype =
  Piqirun_ext.find_piqtype "frame/operand-value-list"
...
let parse_operand_value_list ?opts x (format : Piqirun_ext.input_format) =
  let x_pb =
    Piqirun_ext.convert _operand_value_list_piqtype format `pb x ?opts in
  let buf = Piqirun.init_from_string x_pb
  in Frame_piqi.parse_operand_value_list buf
...
let gen_operand_value_list ?opts x (format : Piqirun_ext.output_format) =
  let buf = Frame_piqi.gen_operand_value_list x in
  let x_pb = Piqirun.to_string buf
  in Piqirun_ext.convert _operand_value_list_piqtype `pb format x_pb ?opts
...
let print_operand_value_list x =
  Pervasives.print_endline (gen_operand_value_list x `piq)
  
let prerr_operand_value_list x =
  Pervasives.prerr_endline (gen_operand_value_list x `piq)
