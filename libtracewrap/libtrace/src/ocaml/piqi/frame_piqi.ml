    ...
    sig
      type t =
        { mutable address : Frame_piqi.address;
          ...
          mutable operand_pre_list : Frame_piqi.operand_value_list;
          mutable operand_post_list : Frame_piqi.operand_value_list option
        }
...
and parse_operand_value_list x = Piqirun.parse_list parse_operand_info x
...
and parse_std_frame x =
  ...
  let (_operand_pre_list, x) =
    Piqirun.parse_required_field 4 parse_operand_value_list x in
  let (_operand_post_list, x) =
    Piqirun.parse_optional_field 5 parse_operand_value_list x
  in
    (Piqirun.check_unparsed_fields x;
     {
       ...
       Std_frame.operand_pre_list = _operand_pre_list;
       Std_frame.operand_post_list = _operand_post_list;
     })
...
and gen__operand_value_list code x =
  Piqirun.gen_list gen__operand_info code x
...
and gen__std_frame code x =
  ...
  let _operand_pre_list =
    Piqirun.gen_required_field 4 gen__operand_value_list
      x.Std_frame.operand_pre_list in
  let _operand_post_list =
    Piqirun.gen_optional_field 5 gen__operand_value_list
      x.Std_frame.operand_post_list
  in
    Piqirun.gen_record code
      [ _address; _thread_id; _rawbytes; _operand_pre_list;
        _operand_post_list ]
...
let gen_operand_value_list x = gen__operand_value_list (-1) x
